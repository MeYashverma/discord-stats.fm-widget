/** Discord widget text fields are short; keep values well under limits. */
export const DEFAULT_TEXT_MAX = 100;
/** Image URLs may be longer (proxy + query). Never ellipsis-truncate them. */
export const IMAGE_URL_MAX = 512;

/** Widget hero slot crops cleanly at this aspect (album art). */
...
/**
 * Prefer a short source URL so the crop proxy stays under Discord's limit.
 * Long CDN/proxy links get reduced to i.scdn.co when possible.
 */
...
/**
 * Force album covers to 480×360 cover-crop so they fit the hero slot
 * (no square art hanging over the rounded widget frame).
 */
export function fitAlbumCoverUrl(url: string): string {
  ...
  // Cover-crop to exactly 480×360. `a=center` keeps the subject in frame.
  const proxied =
  ...
  // Last resort: still try a minimal proxy URL rather than uncropped square art.
  const minimal =
    `https://wsrv.nl/?url=${encodeURIComponent(source)}` +
...
/**
 * Highest-resolution source art URL (uncropped).
 * Display cropping happens later via fitAlbumCoverUrl in the widget payload.
 */
...
/** Stable JSON stringify for payload cache comparisons. */
...
/**
 * Serialize metadata for logs without ever printing secrets.
 * Keys that look like tokens/secrets are redacted.
 */
...
function looksLikeSecret(value: string): boolean {
  // Discord bot tokens are typically three base64-ish segments joined by dots.
  if (/^[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{5,}\.[A-Za-z0-9_-]{20,}$/.test(value)) {
    return true;
  }
  if (/bot\s*token/i.test(value)) return true;
  return false;
}
...
/**
 * Extract a Retry-After delay in milliseconds from common HTTP headers.
 * Falls back to `fallbackMs` when the header is missing or invalid.
 */
